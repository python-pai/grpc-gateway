...
# project proto path
source_p="example/grpc_proto/example_proto"
# service
service_list=("book" "user"  "other")
...
do
  ...
  sed -i "s/from example.grpc_proto.example_proto./from example.python_grpc_proto_code.example.grpc_proto.example_proto./" $target_p/$source_p/$service/*.py
done
